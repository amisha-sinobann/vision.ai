...
def chat():
    try:
        ...
        question = (data.get("question") or data.get("message") or "").strip().lower()
        if not question: return jsonify({"error": "No question provided"}), 400
        
        # Get the latest detection context
        ...
        # 1. CURRENCY
        if any(w in question for w in ["money", "cash", "currency", "rupee", "note", "price"]):
            c = det.get("currency")
        ...
        # 2. COLOUR
        elif any(w in question for w in ["color", "colour", "shade"]):
        ...
        # 3. PEOPLE / PERSON
        elif any(w in question for w in ["people", "person", "human", "someone"]):
            # Filter objects for 'person' label
        ...
        # 4. OBSTACLES / DISTANCE / SAFETY
        elif any(w in question for w in ["safe", "walk", "obstacle", "block", "path"]):
        ...
        # 5. TEXT / READ
        elif any(w in question for w in ["read", "text", "sign", "write"]):
            # Filter for text/OCR objects
            objs = det.get("objects", [])
        ...
        # 6. LOCATION / WHERE / DIRECTIONS
        elif any(w in question for w in ["where", "direction", "left", "right"]):
            ...
            if parts:
                answer = "I see " + "; ".join(parts) + "."
            else:
                answer = "I don't see any objects to locate."

        # DEFAULT FALLBACK: General summary
        else:
            answer = det.get("voice_message") or det.get("scene_summary") or "I am analyzing the scene."

        return jsonify({"answer": answer, "response": answer, "type": "INFO"})
    except Exception as e:
